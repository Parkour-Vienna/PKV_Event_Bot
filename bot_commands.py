...
def check_args(update, context, name, args_needed, relation, args_given):
    if relation(args_given, len(args_needed)):
        message = "The " + name + " command requires " + str(len(args_needed)) + "arguments \n " + name + " " + " ".join(args_needed)
...
def notraining(update, context):
    if check_args(update, context, "/notraining", ["[TIME]", "[LOCATION]"], operator.lt, len(context.args)):
        ...
        try:
            ...
            message = str(user.first_name) + " is no longer training @" + str(loc).capitalize() + ", " + str(timestamp) + " :("
            context.bot.send_message(chat_id=update.effective_chat.id, text=message)
...
def vote(update, context):
    if check_args(update, context, "/vote", ["[EVENT]", "[LOCATION]"], operator.lt, len(context.args)):
        ...
        if user.first_name in [item['who'] for item in voted]:
            ...
            message = str(user.first_name) + " has changed their vote for " + str(event).capitalize() + " to " + str(loc)
            voted.remove(old_vote)
        else:
            message = str(user.first_name) + " has voted for the next " + str(event).capitalize() + " to be at " + str(loc)
